...
class IndicesKFold(object):
    ...
    def __iter__(self):
        ...
        for _ in xrange(self.k):
            ...
            train_index = np.hstack((indices[0: ind],
                indices[ind + self.bin:]))
            ind += self.bin
...
#             print(test_subjects, train_subjects)
            test_index = [ind for ind, x in enumerate(self.subjects) \
                          if x in test_subjects]
            train_index = [ind for ind, x in enumerate(self.subjects) \
                           if x in train_subjects]
            yield train_index, test_index
