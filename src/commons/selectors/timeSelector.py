...
class TimeSelector():
    ...
    def fit(self, X, y, cvIndices=None, timeIndices=None, weights=None):
        self.sections = sectionsUtils.findSigSectionsInPValues(
            ...
            self.sigSectionMinLength, self.timeAxis,
            cvIndices, timeIndices, weights, self.maxSurpriseVal,
            self.labels, self.doPlotSections)

    def transform(self, X):
        return sectionsUtils.concatenateFeaturesFromSections(
            X, self.sections, self.onlyMidValue)
